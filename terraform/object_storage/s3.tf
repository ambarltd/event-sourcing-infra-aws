...
resource "aws_s3_bucket_cors_configuration" "cors_configuration" {
  ...
  cors_rule {
    ...
    allowed_origins = [
      "https://${var.frontend_cors_domain}",
      "https://*.${var.frontend_cors_domain}",
    ]
    expose_headers  = ["ETag"]
...
# Resources in the /public/ directory will be accessible without a presigned url
resource "aws_s3_bucket_policy" "blob_storage_policy" {
  bucket = aws_s3_bucket.blob_storage.id
    ...
    Statement = [
      {
        ...
        Principal = "*"
        Action    = "s3:GetObject"
        Resource  = "${aws_s3_bucket.blob_storage.arn}/public/*"
      }
    ]
  })

  depends_on = [aws_s3_bucket_public_access_block.blob_storage_public_access]
}
